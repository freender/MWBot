...
def command_start_silent(message):
	if not modules.is_owner(message):
		...
		try:
			...
			logging.error(f"Error in start_silent command: {e}", exc_info=True)
...
def command_stop_silent(message):
	if not modules.is_owner(message):
		...
		try:
			...
			error_msg = "❌ Unexpected error occurred. Check logs for details."
			logging.error(f"Error in stop_silent command: {e}", exc_info=True)
			bot.reply_to(message, error_msg)		
...
def command_firmware_mw(message):
	if not modules.is_owner(message):
		...
		try:
			...
			error_msg = "❌ Unexpected error occurred. Check logs for details."
			logging.error(f"Error in firmware_mw command: {e}", exc_info=True)
			bot.reply_to(message, error_msg)
...
def command_reboot_mw(message):
	if not modules.is_owner(message):
		...
		try:
			...
			error_msg = "❌ Unexpected error occurred. Check logs for details."
			logging.error(f"Error in reboot_mw command: {e}", exc_info=True)
			bot.reply_to(message, error_msg)
...
def command_generic_mw(message):
	if not modules.is_owner(message):
		...
		try:
			...
			error_msg = "❌ Unexpected error occurred. Check logs for details."
			logging.error(f"Error in generic_mw command: {e}", exc_info=True)
			bot.reply_to(message, error_msg)
...
def command_stop_mw(message):
	if not modules.is_owner(message):
		...
		try:
			...
			error_msg = "❌ Unexpected error occurred. Check logs for details."
			logging.error(f"Error in stop_mw command: {e}", exc_info=True)
			bot.reply_to(message, error_msg)
