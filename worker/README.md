...
This Worker gives MWBot the client ASN and, when Cloudflare provides it, AS organization.
Sessions last five minutes, are identified by a random 256-bit token, and contain no Telegram identifiers.
...
3. The Worker stores `request.cf.asn` and optional `request.cf.asOrganization` in KV.
